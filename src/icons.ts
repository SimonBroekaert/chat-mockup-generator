...
import { setHtml, trustedHtml, type SafeHtml } from "./html.ts";
...
/** Narrows a string read from the markup (`data-icon="..."`) to a name the tables contain. */
export function isIconName(value: unknown): value is IconName {
    return typeof value === "string" && (Object.hasOwn(STROKE_ICONS, value) || Object.hasOwn(BRAND_ICONS, value));
}
...
/**
 * Fills every [data-icon] placeholder inside root with its SVG. An unknown
 * name gets the generic "message" glyph so a typo degrades visibly instead
 * of throwing.
 */
export function hydrateIcons(root: ParentNode = document): void {
    for (const element of root.querySelectorAll("[data-icon]")) {
        ...
        setHtml(element, icon(isIconName(name) ? name : "message"));
    }
}
